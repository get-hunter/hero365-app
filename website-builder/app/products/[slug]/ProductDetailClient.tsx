  ...
  const calculatePricing = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Fallback: compute simple pricing using installation base price client-side
        const installBase = selectedInstallation.base_install_price || 0;
        const prodSubtotal = product.unit_price * quantity;
        const installSubtotal = installBase * quantity;
        const subtotalBefore = prodSubtotal + installSubtotal;
        const taxRate = 0.08;
        const taxAmount = subtotalBefore * taxRate;
        setPricingBreakdown({
          product_unit_price: product.unit_price,
          installation_base_price: installBase,
          quantity,
          product_subtotal: prodSubtotal,
          installation_subtotal: installSubtotal,
          subtotal_before_discounts: subtotalBefore,
          membership_type: selectedMembership,
          product_discount_amount: 0,
          installation_discount_amount: 0,
          total_discount_amount: 0,
          bundle_savings: 0,
          subtotal_after_discounts: subtotalBefore,
          tax_rate: taxRate,
          tax_amount: taxAmount,
          total_amount: subtotalBefore + taxAmount,
          total_savings: 0,
          savings_percentage: 0,
          formatted_display_price: `$${(subtotalBefore + taxAmount).toFixed(2)}`
        });
      }
    } catch (error) {
    ...
    <>
      ...
      <ProductSchema
        product={{
          ...
          brand: (product as any).brand || 'Professional Grade',
          model: product.sku,
          sku: product.sku,
          category: product.category_name || 'HVAC Equipment',
          specifications: product.technical_specs || {},
          energyRating: product.energy_efficiency_rating,
          images: product.gallery_images || [],
          featuredImage: product.featured_image_url,
          requiresProfessionalInstall: product.requires_professional_install,
          installationService: selectedInstallation ? {
            name: selectedInstallation.option_name,
            description: selectedInstallation.description || 'Professional installation service',
            price: selectedInstallation.base_install_price,
            duration: `${selectedInstallation.estimated_duration_hours} hours`,
            warranty: (selectedInstallation as any).warranty_years ? `${(selectedInstallation as any).warranty_years} years` : '1 year',
            included: (selectedInstallation as any).included_in_install || []
          } : undefined,
          keywords: [
            product.name,
            ...
            businessProfile?.service_areas?.[0] || 'local area'
          ].filter(Boolean),
        ...
        offer={{
          ...
          priceCurrency: 'USD',
          availability: product.is_active && (product.current_stock || 0) > 0 ? 'InStock' : 'OutOfStock',
          priceValidUntil: new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 90 days
          warranty: product.warranty_years ? `${product.warranty_years} years` : undefined
        }}
        seller={{
          name: businessProfile?.business_name || 'Professional Services',
          telephone: businessProfile?.phone || '',
          url: typeof window !== 'undefined' ? window.location.origin : '',
          address: {
            streetAddress: businessProfile?.address || '123 Main St',
            addressLocality: businessProfile?.service_areas?.[0] || 'Austin',
            addressRegion: 'TX',
            postalCode: '78701'
          },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="space-y-4 lg:col-span-5">
          ...
          <div className="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-lg bg-gray-200">
            {allImages.length > 0 ? (
              ...
              <div className="h-80 w-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center">
        ...
        <div className="space-y-6 lg:col-span-7">
          ...
          {/* Membership selector handled below in Member Exclusive Pricing */}
          ...
          <div className="flex items-center space-x-3">
            ...
            <select
              ...
              className="w-24 px-2 py-1.5 text-sm border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
            >
          ...
          {/* Primary Actions - placed near the top for visibility */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3 space-y-3 sm:space-y-0">
            <button
              ...
              className="w-full sm:flex-1 bg-blue-600 text-white font-medium py-2.5 px-4 text-sm rounded-md hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
            >
            ...
            <div className="flex space-x-3 w-full sm:flex-1">
              <a 
                ...
                className="flex-1 border border-gray-300 text-gray-700 font-medium py-2.5 px-4 text-sm rounded-md hover:bg-gray-50 transition-colors text-center"
              >
                Call for Quote
              </a>
              <BookingCTAButton className="flex-1 text-sm py-2.5">
                Schedule Install
              </BookingCTAButton>
            </div>
          </div>
