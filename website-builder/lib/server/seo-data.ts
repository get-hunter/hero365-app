...
export async function loadSEOPages(businessId: string): Promise<void> {
  try {
    ...
    // Disable demo fallback: keep empty maps so UI can decide what to render
    console.log('⚠️ [SEO] No SEO data available (no demo fallback).')
    seoPages = {}
    contentBlocks = {}
    
  } catch (error: any) {
    console.error('❌ [SEO] Unexpected error in loadSEOPages:', error?.message || error)
    seoPages = {}
    contentBlocks = {}
  }
}
...
/**
 * Get location-specific pages for a given service (canonical: /services/{service}/{location})
 */
export async function getLocationPagesForService(serviceSlug: string): Promise<Array<{ url: string; title: string; location_slug: string }>> {
  // Ensure pages are loaded
  if (Object.keys(seoPages).length === 0) {
    const businessId = getBusinessId()
    await loadSEOPages(businessId)
  ...
  const results: Array<{ url: string; title: string; location_slug: string }> = []
  const prefix = `/services/${serviceSlug}/`

  for (const [url, page] of Object.entries(seoPages)) {
    if (url.startsWith(prefix) && page && page.title) {
      const parts = url.split('/').filter(Boolean)
      const location_slug = parts[2] || ''
      if (location_slug) {
        results.push({ url, title: page.title, location_slug })
      }
    }
  }

  // Sort alphabetically by location for stable UI
  results.sort((a, b) => a.location_slug.localeCompare(b.location_slug))
  return results
}
...
/**
 * Get business navigation data
 */
export async function getBusinessNavigation(): Promise<{ services: any[], locations: any[] }> {
  // This would typically come from the generated content or API
 ...
 */
export async function getBusinessLocations(): Promise<any[]> {
  const nav = await getBusinessNavigation()
