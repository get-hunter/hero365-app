-- Ensure every business has business_services entries for ALL of its trades
-- (primary + secondary). We derive service rows from trade_activities.
...
WITH all_business_trades AS (
  ...
  UNION
  -- Secondary trades (array of names mapped to trade_profiles.slug)
...
missing_services AS (
  ...
  JOIN trade_activities ta
    ON ta.trade_slug = abt.trade_slug
